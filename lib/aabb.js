...
class AABB {
  ...
  computeOffsetX (other, offsetX) {
    if (other.maxY > this.minY && other.minY < this.maxY && other.maxZ > this.minZ && other.minZ < this.maxZ) {
      if (offsetX > 0.0 && other.maxX <= this.minX + COLLISION_EPSILON) {
        offsetX = Math.min(Math.max(0, this.minX - other.maxX), offsetX)
      } else if (offsetX < 0.0 && other.minX >= this.maxX - COLLISION_EPSILON) {
  ...
  computeOffsetY (other, offsetY) {
    if (other.maxX > this.minX && other.minX < this.maxX && other.maxZ > this.minZ && other.minZ < this.maxZ) {
      if (offsetY > 0.0 && other.maxY <= this.minY + COLLISION_EPSILON) {
        offsetY = Math.min(Math.max(0, this.minY - other.maxY), offsetY)
      } else if (offsetY < 0.0 && other.minY >= this.maxY - COLLISION_EPSILON) {
  ...
  computeOffsetZ (other, offsetZ) {
    if (other.maxX > this.minX && other.minX < this.maxX && other.maxY > this.minY && other.minY < this.maxY) {
      if (offsetZ > 0.0 && other.maxZ <= this.minZ + COLLISION_EPSILON) {
        offsetZ = Math.min(Math.max(0, this.minZ - other.maxZ), offsetZ)
      } else if (offsetZ < 0.0 && other.minZ >= this.maxZ - COLLISION_EPSILON) {
